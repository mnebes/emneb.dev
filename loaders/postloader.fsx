...
let getConfig (fileContent : string) =
    ...
    let splitKey (line: string) =
        ...
        match line[..seperatorIndex - 1], line[seperatorIndex + 1..] with
        | key, value ->
            if String.IsNullOrWhiteSpace(value.Trim())
            then None
            else Some(key.Trim().ToLower(), value.Trim())
    fileContent
...
let loadFile (rootDir: string) (n: string) =
    ...
    let subtitle = config |> Map.tryFind "subtitle" |> Option.map trimString
